...
test.describe('GET /usuarios', () => {
    ...
    test('it should return 404 for invalid endpoint with query parameters', async ({ request }) => {
    ...
    test('it should return 404 for invalid endpoint with ID and trailing slash', async ({ request }) => {
        const invalidId = '1234567890123456';
        const response = await request.get(`https://serverest.dev/usuarios-invalid-endpoint/${invalidId}/`);
        expect(response.status()).toBe(404);
        const responseBody = await response.json();
    ...
    test('it should return 404 for invalid endpoint with query parameters and trailing slash', async ({ request }) => {
        const response = await request.get('https://serverest.dev/usuarios-invalid-endpoint/?param=value');
        expect(response.status()).toBe(404);
        const responseBody = await response.json();
        expect(responseBody).toHaveProperty('message', 'Endpoint não encontrado');
    ...
    test('it should return 404 for invalid endpoint with ID, query parameters, and trailing slash', async ({ request }) => {
        const invalidId = '1234567890123456';
        const response = await request.get(`https://serverest.dev/usuarios-invalid-endpoint/${invalidId}/?param=value`);
        expect(response.status()).toBe(404);
        const responseBody = await response.json();
