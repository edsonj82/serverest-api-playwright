...
test.describe('GET /carrinhos', () => {
    ...
    test('it should return all shopping carts for the user', async ({ request }) => {
        // 1. Criar um novo usuário admin e obter um token exclusivo
        const user = getUser();
        await request.post('https://serverest.dev/usuarios', { data: user });
        ...
        // 2. Criar um produto dinâmico e válido
        const product = getProduct();
        ...
        // 3. Criar o carrinho com o token e produto isolados
        const createCartResponse = await request.post('https://serverest.dev/carrinhos', {
            data: {
                produtos: [
                    {
...
test.describe('GET /carrinhos/:id', () => {
    ...
    test('it should return when trying to retrieve a shopping cart without authorization', async ({ request }) => {
        const response = await request.get(`https://serverest.dev/carrinhos/${cartId}`);
        console.log('Response body:', await response.json());
...
test.describe('DELETE /carrinhos/concluir-compra', () => {
    ...
    test('it should complete the purchase of a shopping cart successfully', async ({ request }) => {
...
test.describe('DELETE /carrinhos/cancelar-compra', () => {
    ...
    test('it should cancel the purchase of a shopping cart successfully', async ({ request }) => {
        ...
        const responseData = await response.json();
        expect(responseData).toHaveProperty('message', 'Registro excluído com sucesso. Estoque dos produtos reabastecido');
    });
